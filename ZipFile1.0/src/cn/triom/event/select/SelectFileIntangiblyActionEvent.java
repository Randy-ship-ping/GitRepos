...
public class SelectFileIntangiblyActionEvent implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// 获取用户输入的类型
		String keyWord = JOptionPane.showInputDialog(null, "请输入关键字", "提示", JOptionPane.INFORMATION_MESSAGE);
		// 若用户点击取消则退出
		if (keyWord == null) {
			return;
		}
		...
		// 判断是否存在该文件
		if (files.size() == 0) {
			JOptionPane.showMessageDialog(null, "无此相关文件", "错误", JOptionPane.ERROR_MESSAGE);
			return;
		}
		// 设置行数据
